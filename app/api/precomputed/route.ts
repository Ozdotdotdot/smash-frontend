...
const REMOTE_BASE = "https://server.cetacean-tuna.ts.net/precomputed";

export async function GET(request: Request) {
  const { searchParams } = new URL(request.url);
  const remoteUrl = new URL(REMOTE_BASE);

  // Apply defaults for common params, then forward any extras verbatim (e.g., min_entrants).
  ...
  try {
    const res = await fetch(remoteUrl.toString(), {
      cache: "no-store",
      // If the remote endpoint needs CORS headers, proxying avoids browser CORS issues.
    });
    if (!res.ok) {
